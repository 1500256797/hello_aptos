module aptos_call::claim_coin {
    ...
    struct ModuleData has key {
        ...
        aotemaning_event: event::EventHandle<CoinMintingEvent>,
    }


    fun assert_is_admin(addr: address) acquires ModuleData {
        let admin = borrow_global<ModuleData>(@aptos_call).admin_addr;
    ...
    fun init_module(resource_acc: &signer) {
        ...
        move_to(resource_acc, ModuleData {
            ...
            aotemaning_event: account::new_event_handle<CoinMintingEvent>(resource_acc),
        });
    }

    public entry fun issue(admin: &signer) acquires ModuleData {
        assert!(!exists<Capabilities>(@aptos_call), error::already_exists(EALREADY_COIN_CAP));
        let addr = signer::address_of(admin);
        assert_is_admin(addr);
        let data = borrow_global<ModuleData>(@aptos_call);
    ...
    public entry fun enable_minting(admin: &signer, status: bool) acquires ModuleData {
        let addr = signer::address_of(admin);
        assert_is_admin(addr);
        borrow_global_mut<ModuleData>(@aptos_call).minting_enabled = status;
    }
		
    public entry fun set_admin(admin: &signer, admin_addr: address) acquires ModuleData {
        let addr = signer::address_of(admin);
        assert_is_admin(addr);
        borrow_global_mut<ModuleData>(@aptos_call).admin_addr = admin_addr;
    }

    public entry fun mint(user: &signer, amount: u64) acquires Capabilities {
        assert!(exists<Capabilities>(@aptos_call), error::permission_denied(ENO_COIN_CAP));
        let mint_cap = &borrow_global<Capabilities>(@aptos_call).mint_cap;
    ...
    public entry fun burn(user: &signer, amount: u64) acquires Capabilities {
        assert!(exists<Capabilities>(@aptos_call), error::permission_denied(ENO_COIN_CAP));
        let burn_cap = &borrow_global<Capabilities>(@aptos_call).burn_cap;
